...
public class PrestamoLlaveService {
    ...
    public String agregar(String nombreSolicitante, String salon, String turno) {
        ...
        }else if (salon.trim().isEmpty()){
            return "El salon del solicitante es obligatorio";
            // 3. Validar que turno no sea null.
        }else if (turno==null){
            return "El turno del solicitante es obligatorio";
            // 4. Validar que no exista otro registro con el mismo nombreSolicitante.
        }else if (buscarPorNombreSolicitante(nombreSolicitante)!=null){
            return "El nombre del solicitante ya existe";
        }else {
    ...
    public String actualizar(String nombreOriginal, String nombreNuevo, String salon, String turno) {
        ...
        if (nombreOriginal==null){
            return "Primero seleccione un usuario a actualizar";}
            // 2. Validar que nombreNuevo no esté vacío.
        else if (nombreNuevo.trim().isEmpty()){
            return "El nombre del solicitante es obligatorio";
        ...
        }else if (buscarPorNombreSolicitante(nombreNuevo)!=null){
            return "El nombre del solicitante ya existe";
        }else{
            // 5. Buscar el registro original usando nombreOriginal.
            PrestamoLlave usuarioactualizar =  buscarPorNombreSolicitante(nombreOriginal);
            // 8. Si todo está bien, actualizar los atributos del objeto encontrado.
            if (usuarioactualizar!=null){
                usuarioactualizar.setSalon(salon);
                usuarioactualizar.setTurno(turno);
                usuarioactualizar.setNombreSolicitante(nombreNuevo);
    ...
    public String eliminar(String nombreSolicitante) {
        ...
        if (nombreSolicitante==null){
            return "Seleccione un usuario a eliminar";
        }else{
            // 2. Buscar si existe el registro.
            if (repository.eliminarPorNombreSolicitante(nombreSolicitante)){
                return null;
            }else{
