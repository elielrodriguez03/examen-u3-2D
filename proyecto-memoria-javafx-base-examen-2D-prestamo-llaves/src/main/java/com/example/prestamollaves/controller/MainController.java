...
public class MainController {
    ...
    public void agregar() {
        ...
        String error = service.agregar(nombreSolicitante, salon, turno);
        ...
        actualizarLista();
        limpiar();
        mostrarMensaje("exito", "se registro correctamente", Alert.AlertType.INFORMATION);
    }
    ...
    public void actualizar() {
        if (nombreOriginal== null){
            mostrarMensaje("aviso", "seleccione registro para actualizar", Alert.AlertType.WARNING);
            return;
        }
        String nuevonombre = txtNombreSolicitante.getText();
        String nombresasalon= txtSalon.getText();
        String nuevoturno = cbTurno.getValue();

        String error = service.actualizar(nombreOriginal, nuevonombre, nombresasalon, nuevoturno);
        if (error != null){
            mostrarMensaje("error", error, Alert.AlertType.ERROR);
            return;
        ...
        actualizarLista();
        limpiar();
        mostrarMensaje("exito", "Cse actualizo correctamnete", Alert.AlertType.INFORMATION);
    }

    @FXML
    public void eliminar() {
        String nombreEliminar = txtNombreSolicitante.getText();
        String error = service.eliminar(nombreEliminar);
        if (error!=null){
            mostrarMensaje("error", error, Alert.AlertType.ERROR);
            return;
        }
        actualizarLista();
        limpiar();
        mostrarMensaje("exito", "registro eliminado correctamnete", Alert.AlertType.INFORMATION);
    }
    ...
    private String mostrarMensaje(String titulo, String mensaje, Alert.AlertType tipo) {
